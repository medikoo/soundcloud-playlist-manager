...
var isNumber   = Number.isNumber
  ...
  , forHistory, doHistory = true;

forHistory = function (fn) {
  return function () {
    doHistory = false;
    fn();
    doHistory = true;
...
module.exports = ee({
  ...
  insert: function (item, index) {
    ...
    this.emit('update', { target: item, action: 'insert',
      undo: doHistory && forHistory(this.remove.bind(this, item)),
      redo: doHistory && forHistory(this.insert.bind(this, item, index)) });
  },
  remove: function (item) {
    ...
    if (index >= 0) {
      ...
      this.emit('update', { target: item, action: 'remove',
        undo: doHistory && forHistory(this.insert.bind(this, item, index)),
        redo: doHistory && forHistory(this.remove.bind(this, item)) });
    }
  },
